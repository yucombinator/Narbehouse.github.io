...
const SHAPES = new Set(['daisy', 'cup', 'bell', 'puff', 'spike', 'star', 'wild', 'rosette', 'poppy']);
...
test('same-shape variants still look distinct (unique petal colour per shape)', () => {
  ...
  assert.deepEqual(Object.keys(byShape).sort(), ['bell', 'cup', 'daisy', 'poppy', 'puff', 'rosette', 'spike', 'star', 'wild']);
  for (const [shape, group] of Object.entries(byShape)) {
    ...
    if (shape !== 'spike' && shape !== 'rosette' && shape !== 'poppy') { // lone or small signature families
