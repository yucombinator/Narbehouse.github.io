...
import { initRender, resize, MEADOW_THEMES } from './render.js?v=25';
...
extra.push({
        ...
        faceTiltX: (rng() - 0.5) * 0.7,
        faceTiltZ: (rng() - 0.5) * 0.7,
    ...
    offers.forEach((f, oi) => {
      for (let k = 0; k < 3; k++) {
        ...
        extra.push({
          ...
          faceTiltX: (((i * 0.37 + k * 0.73) % 1) - 0.5) * 0.7,
          faceTiltZ: (((i * 0.29 + k * 0.53) % 1) - 0.5) * 0.7,
          kind: variantForShape(f.shape),
          kindIndex: 0,
          cluster: -(i + 1),
        });
